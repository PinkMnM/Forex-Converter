...
class FlaskTests(TestCase):
    ...
    def __init__(self, method_name: str = "test_html_post"):
        ...
        self.tests = (
            ...
            ((missing_argument_error("amount"),), {"from": "USD", "to": "USD"}),
            ((missing_argument_error("to"),), {"from": "USD", "amount": 15}),
            ((missing_argument_error("from"),), {"to": "USD", "amount": 15}),
            (
                (missing_argument_error("from"), missing_argument_error("amount")),
                {"to": "USD"},
            ),
            (
                (
    ...
    def common_test(
        self, test: Tuple[int, Mapping[str, Union[str, int]]], response: Response
    ):
    ...
    def test_html_post(self):
        ...
        with app.test_client() as client:
            response = client.post(
                "/", headers={"accept": "text/html"}, data=self.tests[0][1]
            )
    ...
    def test_json_post(self):
        ...
        with app.test_client() as client:
            for test in self.tests:
                response = client.post(
                    "/", headers={"accept": "application/json"}, json=test[1]
                )
                self.common_test(test, response)
